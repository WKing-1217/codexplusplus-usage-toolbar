...
安装与更新步骤请看 [首页](../README.md)。本文提供兼容范围、故障排查和开发资料。

Codex++ API 顶部用量栏：查看 API 钱包余额或当前密钥套餐，以及本机 API 任务 Token、缓存和时钟。

**版本：1.0.9。** 独立用户脚本项目，依赖 Codex++ 的脚本加载功能，不是 Codex++ 客户端分发包，也不是 OpenAI 官方扩展。

## 能显示什么

- **钱包计费**：当前查询密钥对应的钱包剩余余额。
- **套餐计费**：接口返回的套餐名称、剩余额度、日／周周期明细和有效期。不会凭供应商名称判断是否属于套餐。
...
| Codex++ | 1.2.56 用户脚本机制 |
| Codex 内置版本 | 已验证 26.831.20005、26.901.20858、26.901.31953；不设版本号白名单 |
| API 接口 | Sub2API `/v1/usage`、New API `/api/usage/token` |

脚本内置 Codex preload 的稳定消息解码协议，不导入带哈希的客户端内部模块，也不按版本号停用。客户端升级后会直接继续运行；如果未来底层消息协议发生实际变化，脚本会报告通信错误，再针对协议变更适配。

## 安装

1. 已安装上表所列版本的 Codex++。
2. 从 [最新发布页](https://github.com/WKing-1217/codexplusplus-usage-toolbar/releases/latest) 下载 `codexplusplus-usage-toolbar-v1.0.9.zip`，**先完整解压**。
...
1. 确认重新加载脚本后底部显示 **1.0.9**。仍显示旧版本号，说明窗口还在运行旧脚本。
...
- API 余额和当前任务 Token 的刷新按钮分别位于各自标题右侧，统计卡片采用大号名称与数值，统一对齐；套餐不再单独显示“每月剩余”行。
- 余额更新时间显示在余额区底部，Token 更新时间保留在 Token 区；页脚只显示版本号。没有缓存写入数据时不显示该说明。
